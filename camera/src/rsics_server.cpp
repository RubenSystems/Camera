...
BroadcastServer::BroadcastServer() {
	rsics_make_listener( "5253" , &listener_);
}
...
void BroadcastServer::start() {
	...
	server_thread_ = std::thread([this](){
		struct message_callback callback = create_message_callback(this, &client_connection);
...
bool BroadcastServer::has_clients() {
	return clients.size() > 0;
}

void BroadcastServer::client_connection(void * s, struct connection * client, message_type type, void * message, uint64_t message_length) {
	connection c = *client;
	char uid [37]; 
...
void BroadcastServer::add_client(const std::string & uid, const connection & comp) {
	Client * new_client = new Client(uid, comp);
	clients[uid] = new_client;
}

void BroadcastServer::renew_client(const std::string & uid, const connection & comp) {
	clients[uid]->renew();
}

void BroadcastServer::recieved_client(const std::string & uid, const connection & comp) {
